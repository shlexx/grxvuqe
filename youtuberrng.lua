...
TextButton.MouseButton1Click:Connect(function()
	...
	local args = t:split(" ")
	local children = game:GetService("ReplicatedStorage").Youtuber_titles:GetChildren()

	if args[1] == "equip" then
		for i = 1, #children do
			local aura = children[i]
			if args[2] == aura.Name or args[2] .. " " .. args[3] == aura.Name then
				game:GetService("ReplicatedStorage").EquipTitle:FireServer(aura.Name)
			end
		end
	end
end)
