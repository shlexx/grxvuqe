...
AL.Name = string.gsub(string.rep(".", 100), ".", function() return utf8.char(math.random(32, 126)) end)
...
Title.Text = "edge's audio logger (remade by roblox.stud <3)"
...
local function stripPrefix(id)
	if string.sub(id, 1, 32) == "http://www.roblox.com/asset/?id=" then
		return string.sub(id, 33)
	elseif string.sub(id, 1, 33) == "https://www.roblox.com/asset/?id=" then
		return string.sub(id, 34)
	end
	return id
end

-- Returns the SoundId string from either a Sound or AudioPlayer instance, or nil if neither
local function getSoundId(child)
	if child:IsA("Sound") then
		return child.SoundId
	elseif child:IsA("AudioPlayer") then
...
function getaudio(place)
	if running == false then
		...
		for _, child in pairs(place:GetDescendants()) do
			spawn(function()
				local soundId = getSoundId(child)
				if soundId and not GUI.Logs:FindFirstChild(soundId) and not FindTable(ignore, soundId) then
					local id = stripPrefix(string.match(soundId, "rbxasset://sounds.+") or string.match(soundId, "&hash=.+") or string.match(soundId, "%d+"))
					if id ~= nil then		
						...
						local data = Instance.new('StringValue') data.Parent = newsound data.Value = soundId data.Name = 'ID'
...
itemadded = game.DescendantAdded:connect(function(added)
	wait()
	local soundId = getSoundId(added)
	if autoscan == true and soundId and not GUI.Logs:FindFirstChild(soundId) and not FindTable(ignore, soundId) then
		local id = stripPrefix(string.match(soundId, "rbxasset://sounds.+") or string.match(soundId, "&hash=.+") or string.match(soundId, "%d+"))
		if id ~= nil then		
			local newsound = GUI.Audio:Clone()
			if string.sub(id, 1, 6) == "&hash=" or string.sub(id, 1, 7) == "&0hash=" then
...
GUI.Info.Listen.MouseButton1Click:Connect(function()
	if GUI.Info.Listen.Text == 'Listen' then
		...
		samplesound.Looped = true samplesound.SoundId = selectedaudio samplesound:Play() samplesound.Name = '🔊'
